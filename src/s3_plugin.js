...
module.exports = class S3Plugin {
  ...
  setupProgressBar(uploadFiles) {
    const progressTotal = uploadFiles.reduce(
      (acc, { upload }) => upload.totalBytes + acc,
      0
    );
  ...
  uploadFile(fileName, file) {
    console.warn("UPLOAD FILE", fileName, file);
    let Key = this.options.basePath + fileName;
    const s3Params = _.mapValues(this.uploadOptions, (optionConfig) => {
      return _.isFunction(optionConfig)
        ? optionConfig(fileName, file)
        : optionConfig;
    });
