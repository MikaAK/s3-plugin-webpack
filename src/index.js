...
export default class S3Plugin {
  ...
  apply(compiler) {
    ...
    compiler.plugin('after-emit', (compilation, cb) => {
      ...
      if (!hasRequiredUploadOpts) { error = `S3Plugin-RequiredS3UploadOpts: ${REQUIRED_S3_UP_OPTS.join(', ')}`; }
  ...
  addPathToFiles(files, fPath) {
    return files.map((file) => { return { name: file, path: path.resolve(fPath, file) }; });
  }

  getFileName(file = '') {
    if (_.includes(file, PATH_SEP)) { return file.substring(_.lastIndexOf(file, PATH_SEP) + 1); }
    return file;
  }

  getAssetFiles({ assets }) {
    const files = _.map(assets, (value, name) => { return { name, path: value.existsAt }; });
  ...
  filterAllowedFiles(files) {
    return files.reduce((res, file) => {
      if (this.isIncludeAndNotExclude(file.name) && !this.isIgnoredFile(file.name)) { res.push(file); }
  ...
  invalidateCloudfront() {
    ...
    return new Promise((resolve, reject) => {
      if (cloudfrontInvalidateOptions.DistributionId) {
        ...
        if (accessKeyId && secretAccessKey) { cloudfront.config.update({ accessKeyId, secretAccessKey }); }
